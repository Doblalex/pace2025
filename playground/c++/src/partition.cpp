...
PartitionRefinement::PartitionRefinement(Graph* graph, globalprops* props)
	: graph(graph), props(props) {
...
bool PartitionRefinement::dorefine() {
	BGL_FORALL_VERTICES(v, *graph, Graph) {
		...
		BGL_FORALL_ADJ_T(v, neighbor, *graph, Graph) { ids.emplace_back((*graph)[neighbor].id); }
		refine(ids);
		bool found =
				!(find(start->elements.begin(), start->elements.end(), 7) == start->elements.end());
	}
	return true;
}
...
void PartitionRefinement::refine(vector<Vertex> arr) {
	...
	for (auto x : arr) {
		...
		if (el->elements[vectorElementAt[x]] != x) {
			exit(0);
		}
