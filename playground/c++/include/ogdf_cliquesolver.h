...
static int build_simple_graph_instance_from_pointer_to_edge_list(int* inputEdgesFrom,
		int* inputEdgesTo, int numVertices, int numEdges) {
	...
	for (i = 1; i <= NB_NODE; i++) {
		...
		none_neibors[i] = (int*)malloc((NB_NODE) * sizeof(int));
		nb1 = 0;
		nb2 = 0;
		for (j = 1; j <= NB_NODE; j++) {
			if (matrice[i][j] == TRUE) {
...
bool solveMISInstanceWithCliqueSolver(Instance& I, long limit, bool docheck = false) {
		...
		I.forAllCanDominate(indexToNode[i], [&](ogdf::node adj) {
			I.forAllCanBeDominatedBy(adj, [&](ogdf::node adj2) {
				if (adj2 != indexToNode[i]) {
					adjMatrix[i][nodeToIndex[adj2]] = false;
					--numEdges;
				}
				return true;
			});
	...
	build_simple_graph_instance_from_pointer_to_edge_list(edgesFrom.data(), edgesTo.data(),
	...
	for (i = 0; i < MAX_CLQ_SIZE; i++) {
		...
		clique.insert(indexToNode[solution[i] - 1]);
	}

	for (auto it = I.G.nodes.begin(); it != I.G.nodes.end(); it++) {
		if (!I.is_subsumed[*it] && clique.find(*it) == clique.end()) {
			I.DS.insert(I.node2ID[*it]);
		}
	}
	return true;
}
