...
#	define MY_LIB_IGNORE_DEPRECATED_END _Pragma("GCC diagnostic pop")
...
#	define MY_LIB_IGNORE_DEPRECATED_END _Pragma("warning(pop)")
...
public:
	...
	BufferStdout(std::string& collector)
		: m_collector(collector), fp(std::fopen("output.txt", "w")) {
		if (fp == nullptr) {
	...
	~BufferStdout() {
		...
		if (std::ifstream is("output.txt"); is) {
			m_collector.append(std::istreambuf_iterator<char>(is), std::istreambuf_iterator<char> {});
		}
...
typedef boost::adjacency_list<boost::listS, boost::listS, boost::undirectedS,
		boost::property<boost::vertex_index_t, size_t, vertex_info>>
		Graph;
