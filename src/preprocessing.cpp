	...
	for (boost::tie(vi, vend) = boost::vertices((*instance->G)); vi != vend;) {
		...
		if (boost::out_degree(v, (*instance->G)) == 1) {
			...
			if ((*instance->G)[v].is_dominated
					&& ((*instance->G)[w].can_be_dominating_set || (*instance->G)[w].is_dominated)) {
				vi++;
...
list<Instance*> decomposeConnectedComponents(Instance* instance) {
	...
	int cnt = 0;
	BGL_FORALL_VERTICES(v, *instance->G, Graph) { boost::put(index_map, v, cnt++); }
...
bool reductionDomination(Instance* instance) {
	...
	BGL_FORALL_VERTICES(v, *instance->G, Graph) {
		BGL_FORALL_ADJ_T(v, w, *instance->G, Graph) { is_neighbor[(*instance->G)[w].id] = true; }
		BGL_FORALL_ADJ_T(v, w, *instance->G, Graph) {
			...
			if (dominated && (*instance->G)[w].can_be_dominating_set == true
					&& (*instance->G)[v].can_be_dominating_set == true) {
				(*instance->G)[w].can_be_dominating_set = false;
				reduced = true;
			}
		}
		BGL_FORALL_ADJ_T(v, w, *instance->G, Graph) { is_neighbor[(*instance->G)[w].id] = false; }
	}
	...
	while (partitionElement != NULL) {
		if (partitionElement->elements.size() > 1) {
			...
			for (size_t i = 0; i < partitionElement->elements.size(); i++) {
				...
				if ((*instance->G)[v1].can_be_dominating_set
						&& !(*instance->G)[v1].is_dominated) { // TODO: keep this safe for now, maybe can be improved. But without this we get wrong results
					keep = i;
					break;
				}
			}
...
bool reductionDominationPaper(Instance* instance, VertexList& dominatingSet) {
	...
	for (boost::tie(vi, vend) = boost::vertices((*instance->G)); vi != vend;) {
		...
		if (!someonedominated) {
			...
			BGL_FORALL_ADJ_T(v, w, *instance->G, Graph) { visited[(*instance->G)[w].id] = 0; }
			vi++;
			continue;
		}
		list<VD> N1v;
